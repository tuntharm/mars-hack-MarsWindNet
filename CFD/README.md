...
Start with [the north-up map](geometry/city-map.svg), [canonical city](geometry/city.json), [14 obstacles](geometry/obstacles.csv) and [5 sensors](geometry/sensors.csv). Layout identity is `marswindnet-400-v2`. The app imports this geometry into `public/data/city/marswindnet-layout-v2.json`; run `npm run check:geometry` to check agreement. Keep positions, dimensions, IDs and roles consistent.
...
Domain `[0,400] × [0,400]` metres; southwest origin; x east, y north. The display grid is **not the computational mesh**:

```text
nx = ny = 128; dx = dy = 3.125 m
i,j = 0..127
index = j*128+i
x = (i+0.5)*3.125
y = (j+0.5)*3.125
...
Rows run west-to-east, then south-to-north. First/last centres are 1.5625/398.4375 m. Every array has 16,384 entries. All valid cells require finite u/v in m/s. Masked cells may be null. `is_fluid` must be booleans. The importer combines your mask with the canonical obstacles.
...
| S3 | 108,100 | Independent checkpoint, withheld from prediction |
