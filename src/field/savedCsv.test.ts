...
describe('shipped illustrative paired CSV', () => {
  ...
  it('ships the varied pair and exact-coordinate observations under the current layout', () => {
    ...
    const expected = evaluateIllustrativeVelocity(loadTestCity(), 1.5625, 1.5625, 8, 2)
    ...
    expect(observations.readings[0]).toMatchObject({ sensor_id: 'S1', x_m: 0, y_m: 400 })
    expect(observations.readings[0].u_mps).toBe(evaluateIllustrativeVelocity(loadTestCity(), 0, 400, 8, 2).u)
  })
})
